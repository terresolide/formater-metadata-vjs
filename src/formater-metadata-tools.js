...
//		    }
         ...
         if (process.env.GEONETWORK ) {
           // default search json
           var json = process.env.GEONETWORK + 'catalog/locales/' + this.$locale + '-search.json'
           this.$http.get(json).then(
                response => { Vue.prototype.$gnLang = Object.assign(this.$gnLang,response.body)}
            )
            ...
            }).then( response => { Vue.prototype.$gnLang = Object.assign(this.$gnLang,response.body)})
           // group name
